...
export type TimerPhase = 'IDLE' | 'COUNTDOWN' | 'WORK' | 'REST' | 'ROUND_REST' | 'FINISHED';
...
export function useIntervalTimer() {
  ...
  const calculateTotalTime = useCallback((cfg: TimerConfig) => {
    const oneRound = (cfg.exercises * (cfg.workDuration + cfg.restDuration)) - cfg.restDuration; // Last rest of round is roundRest
    const total = (oneRound * cfg.rounds) + ((cfg.rounds - 1) * cfg.roundRestDuration);
  ...
  const tick = useCallback(() => {
    setState(prev => {
      if (prev.timeLeft > 1) {
        return { ...prev, timeLeft: prev.timeLeft - 1, elapsedTime: prev.phase !== 'COUNTDOWN' ? prev.elapsedTime + 1 : prev.elapsedTime };
      }

      // Time left is 0 (or 1 going to 0), transition phase
      const { phase, currentRound, currentExercise } = prev;
      const { rounds, exercises, workDuration, restDuration, roundRestDuration } = config;
      ...
      return {
        ...
        elapsedTime: prev.phase !== 'COUNTDOWN' ? prev.elapsedTime + 1 : prev.elapsedTime
      };
    });
  }, [config]);

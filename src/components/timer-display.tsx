...
export function TimerDisplay({ state, config, onStop }: TimerDisplayProps) {
  const getPhaseColor = () => {
    switch (state.phase) {
      case 'WORK': return 'text-green-600 dark:text-green-400';
      case 'REST': return 'text-amber-600 dark:text-amber-400';
      case 'ROUND_REST': return 'text-blue-600 dark:text-blue-400';
      case 'COUNTDOWN': return 'text-purple-600 dark:text-purple-400';
  ...
  const getPhaseLabel = () => {
    switch (state.phase) {
      ...
      case 'ROUND_REST': return 'ROUND REST';
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] w-full max-w-md mx-auto space-y-8">
      <div className="text-center space-y-2">
        <h2 className={`text-4xl font-black tracking-tighter ${getPhaseColor()} animate-in fade-in zoom-in duration-300`}>
          {getPhaseLabel()}
        </h2>
        <div className="text-8xl font-black tabular-nums tracking-tighter text-slate-900 dark:text-slate-50">
      ...
      <div className="w-full space-y-2">
        ...
        <div className="flex justify-between text-sm text-muted-foreground font-medium">
          <span>Round {state.currentRound} / {config.rounds}</span>
          <span>Exercise {state.currentExercise} / {config.exercises}</span>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-4 w-full pt-8">
         {/* Placeholder for Pause/Resume if we implement it later */}
        <Button variant="outline" size="lg" className="w-full h-14 text-lg" onClick={onStop}>
          <X className="mr-2 h-5 w-5" /> Stop
        </Button>
        {state.phase === 'FINISHED' && (
             <Button size="lg" className="w-full h-14 text-lg bg-green-600 hover:bg-green-700" onClick={onStop}>
             <RotateCcw className="mr-2 h-5 w-5" /> New Workout
           </Button>
        )}
      </div>
    </div>
